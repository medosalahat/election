...
<div class="col-sm-3 col-md-2 sidebar">
    <ul class="nav nav-sidebar">
        <li class="active"><a href="<?= site_url('admin') ?>">Home</a></li>
...
<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
    <h2 class="page-header">Number election ( (  <?=$class->count_election()?> ) )</h2>
    <div class="row placeholders">

        <?PHP foreach ($data as $row) { ?>

            <div class="col-xs-6 col-sm-3 placeholder">
                <img src="<?= site_url($row[$tpl_students->image()]) ?>" width="200"
                     height="200" class="img-responsive" alt="Generic placeholder thumbnail">
                <h4><?= $row['sum'] ?></h4>
                <span
                    class="text-muted"><?= $row[$tpl_students->first_name()] . ' ' . $row[$tpl_students->first_name()] ?></span>
            </div>
        <?php } ?>
    </div>
